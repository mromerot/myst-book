...
The current toolchain used by [JupyterBook](https://jupyterbook.org/) is based on [Sphinx](https://www.sphinx-doc.org/), which is an open-source documentation system used in many software projects, especially in the Python ecosystem. `mystjs` is a similar tool to [Sphinx](https://www.sphinx-doc.org/), however, designed specifically for scientific communication. In addition to building websites, `mystjs` can also help you create scientific PDFs, Microsoft Word documents, and JATS XML (used in scientific publishing).

`mystjs` uses existing, modern web-frameworks in place of the [Sphinx](https://www.sphinx-doc.org/) build system. These tools come out-of-the-box with prefetching for faster navigation, smaller network payloads through modern web-bundlers, image optimization, partial-page refresh through single-page application. Many of these features, performance and accessibility improvements are difficult, if not impossible, to create inside of the [Sphinx](https://www.sphinx-doc.org/) build system.
...
The `mystjs` ecosystem was developed as a collaboration between [Curvenote](https://curvenote.com), [2i2c](https://2i2c.org/) and the [ExecutableBooks](https://executablebooks.org/) team. The initial version of `mystjs` was originally release by [Curvenote](https://curvenote.com) as the [Curvenote CLI](https://curvenote.com/docs/cli) under the MIT license, and transferred to the [ExecutableBooks](https://executablebooks.org/) team in October 2022. The goal of the project is to enable the same rich content and authoring experiences that [Sphinx](https://www.sphinx-doc.org/) allows for software documentation, with a focus on web-first technologies (Javascript), interactivity, accessibility, scientific references (e.g. DOIs and other persistent IDs), professional PDF outputs, and JATS XML documents for scientific archiving.
...
![](images/citations.png) **Figure 1**: Citations are rendered with a popup directly inline.
...
![](images/equations.gif) **Figure 2**: In context cross-references improve the reading experience.
...
![](images/interactive.gif) **Figure 3**: Embedding data, interactivity and computation into a MyST article.
...
![](images/structured-data.gif) **Figure 4**: The data behind MyST is **structured**, which means we can transform it into many different document types and use it to power all sorts of exciting features!
...
![](images/pdf-two-column.png) **Figure 5**: A PDF rendering through MyST.
